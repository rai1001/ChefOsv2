...
function TempBadge({ log }: { log: TemperatureLog }) {
  if (log.is_within_range === null)
    return <span className="text-lg font-mono font-bold text-text-secondary">{formatTemp(log)}</span>;
  if (log.is_within_range)
    return (
      <span className="inline-flex items-center gap-1 text-success font-mono font-bold">
        <CheckCircle2 className="h-4 w-4" /> {formatTemp(log)}
      </span>
    );
  return (
    <span className="inline-flex items-center gap-1 text-danger font-mono font-bold">
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-text-primary">Registro de temperaturas</h1>
          <p className="text-sm text-text-muted mt-0.5">Últimas 24 horas</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-bg-card border border-border rounded-md p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1">Temperatura (°C)</label>
              <input
                type="number"
                step="0.1"
                value={temperature}
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1">Mín. permitida</label>
                <input
                  type="number"
                  step="0.1"
                  value={minAllowed}
              ...
              <div>
                <label className="block text-xs font-medium text-text-secondary mb-1">Máx. permitida</label>
                <input
                  type="number"
                  step="0.1"
                  value={maxAllowed}
      ...
      <div className="flex gap-2 flex-wrap">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${!filterLocation ? 'bg-accent text-white border-accent' : 'border-border hover:bg-bg-sidebar'}`}
        >
        ...
        {[...new Set(logs.map(l => l.location))].map(loc => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${filterLocation === loc ? 'bg-accent text-white border-accent' : 'border-border hover:bg-bg-sidebar'}`}
          >
            {loc}
          </button>
        ))}
        ...
        <div className="bg-bg-card border border-border rounded-md overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-bg-sidebar border-b border-border">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-text-secondary">Hora</th>
                <th className="text-left px-4 py-3 font-medium text-text-secondary">Ubicación</th>
                <th className="text-left px-4 py-3 font-medium text-text-secondary">Temperatura</th>
                <th className="text-left px-4 py-3 font-medium text-text-secondary hidden md:table-cell">Rango</th>
                <th className="text-left px-4 py-3 font-medium text-text-secondary hidden lg:table-cell">Notas</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {logs.map(log => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium text-text-primary">{log.location}</td>
                  ...
                  <td className="px-4 py-3 text-text-muted hidden md:table-cell">
                    {log.min_allowed !== null || log.max_allowed !== null
                      ...
                      : <span className="text-text-muted">Sin límites</span>}
                  </td>
                  <td className="px-4 py-3 text-text-muted hidden lg:table-cell">{log.notes ?? <span className="text-text-muted">—</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
