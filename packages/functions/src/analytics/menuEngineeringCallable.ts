...
export const getMenuAnalytics = onCall(async (request) => {
  ...
  await checkRateLimit(uid, 'get_menu_analytics');

  const db = admin.firestore();

  try {
    let eventsQuery: admin.firestore.Query = db
      .collection('events')
    ...
    eventsSnap.forEach((doc) => {
      ...
      const recipesInMenu = menu.recipeIds
        .map((rid: string) => ({
          id: rid,
    ...
    logError('Menu Analytics Error:', error, { uid, outletId });
    throw new HttpsError('internal', 'Analytics calculation failed.');
  }
});
