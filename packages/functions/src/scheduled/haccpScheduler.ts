    ...
    for (const outletDoc of outletsSnap.docs) {
      ...
      try {
        ...
        const completionsSnap = await db
          .collection('haccpTaskCompletions')
          .where('outletId', '==', outletId)
        ...
        logsSnap.docs.forEach((d) => {
          ...
          doc
            .fontSize(10)
            .text(
              `${format(new Date(l.timestamp), 'dd/MM HH:mm')} - ${l.pccId}: ${l.value}°C (${l.status})`
            );
        });
        doc.moveDown();
        ...
        completionsSnap.docs.forEach((d) => {
          ...
          doc
            .fontSize(10)
            .text(
              `${format(new Date(c.completedAt), 'dd/MM')} - Task ID: ${c.taskId} - Por: ${c.completedBy}`
            );
        });

        doc.moveDown();
        doc.fontSize(10).text('--- Fin del Reporte ---', { align: 'center' });
        doc.text(`Generado automáticamente el ${format(now, 'dd/MM/yyyy HH:mm')}`, {
          align: 'center',
        });
        ...
        const pdfBuffer = Buffer.concat(
          await new Promise<any[]>((resolve) => doc.on('end', () => resolve(buffers)))
        );
