...
export const monitorHACCP = onDocumentCreated('haccpLogs/{logId}', async (event) => {
  ...
  const lastLogsSnap = await admin
    .firestore()
  ...
  const limits = pcc
    ? `Min Temp: ${pcc.minTemp ?? 'N/A'}, Max Temp: ${pcc.maxTemp ?? 'N/A'}`
    : 'Unknown limits';
