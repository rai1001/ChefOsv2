 ...
 */
export const processExcelImport = onObjectFinalized({}, async (event) => {
  ...
  try {
    ...
    const processIngredientRow = (row: any) => {
      ...
      const price = parseFloat(
        String(row['Precio'] || row['Coste'] || row['Price'] || '0').replace(',', '.')
      );
    ...
    if (jsonData) {
      ...
      const rows = Array.isArray(jsonData)
        ? jsonData
        : (jsonData as any).ingredients || (jsonData as any).items || [];
      ...
      for (const sheetName of workbook.SheetNames) {
        ...
        const isRecipeSheet =
          sheetName.toUpperCase().includes('RECETA') ||
          sheetName.toUpperCase().includes('PLATO') ||
          sheetName.toUpperCase().includes('MENU');

        const isSupplierSheet =
          sheetName.toUpperCase().includes('PROVEEDOR') ||
        ...
        const isOccupancySheet =
          sheetName.toUpperCase().includes('OCUPACION') ||
        ...
        if (isMasterList) {
          ...
          json.forEach((row: any) => {
            ...
            const qty = parseFloat(
              String(row['Cantidad'] || row['Quantity'] || '0').replace(',', '.')
            );
