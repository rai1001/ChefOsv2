...
import { IProductionRepository, FirebaseProductionRepository } from '@/infrastructure/repositories/FirebaseProductionRepository';
import { IPurchasingRepository, FirebasePurchasingRepository } from '@/infrastructure/repositories/FirebasePurchasingRepository';
...
export function bootstrap() {
  ...
  container.bind<LoginWithEmailUseCase>(TYPES.LoginWithEmailUseCase).to(LoginWithEmailUseCase).inTransientScope();

  // Services
  container.bind<IIngredientRepository>(TYPES.IngredientRepository).to(FirebaseIngredientRepository).inSingletonScope();
  container.bind<IRecipeRepository>(TYPES.RecipeRepository).to(FirebaseRecipeRepository).inSingletonScope();
  ...
  // Recipe Use Cases
  container.bind<CalculateRecipeCostUseCase>(TYPES.CalculateRecipeCostUseCase).to(CalculateRecipeCostUseCase).inTransientScope();
  container.bind<GetRecipesUseCase>(TYPES.GetRecipesUseCase).to(GetRecipesUseCase).inTransientScope();
  container.bind<CreateRecipeUseCase>(TYPES.CreateRecipeUseCase).to(CreateRecipeUseCase).inTransientScope();
  container.bind<UpdateRecipeUseCase>(TYPES.UpdateRecipeUseCase).to(UpdateRecipeUseCase).inTransientScope();
  container.bind<DeleteRecipeUseCase>(TYPES.DeleteRecipeUseCase).to(DeleteRecipeUseCase).inTransientScope();

  // Inventory Use Cases
  container.bind<IInventoryRepository>(TYPES.InventoryRepository).to(FirebaseInventoryRepository).inSingletonScope();
  container.bind<RegisterStockMovementUseCase>(TYPES.RegisterStockMovementUseCase).to(RegisterStockMovementUseCase).inTransientScope();
  container.bind<GetInventoryStatusUseCase>(TYPES.GetInventoryStatusUseCase).to(GetInventoryStatusUseCase).inTransientScope();
  container.bind<PerformAuditUseCase>(TYPES.PerformAuditUseCase).to(PerformAuditUseCase).inTransientScope();

  // Analytics Use Cases
  container.bind<CalculateBCGMatrixUseCase>(TYPES.CalculateBCGMatrixUseCase).to(CalculateBCGMatrixUseCase).inTransientScope();

  // Schedule & Events Use Cases
  container.bind<GenerateScheduleUseCase>(TYPES.GenerateScheduleUseCase).to(GenerateScheduleUseCase).inTransientScope();
  container.bind<ParseMatrixPlaningUseCase>(TYPES.ParseMatrixPlaningUseCase).to(ParseMatrixPlaningUseCase).inTransientScope();
  container.bind<ParseServiceSheetUseCase>(TYPES.PARSE_SERVICE_SHEET_USE_CASE).to(ParseServiceSheetUseCase).inTransientScope();
  container.bind<ScanSportsMenuUseCase>(TYPES.SCAN_SPORTS_MENU_USE_CASE).to(ScanSportsMenuUseCase).inTransientScope();
  container.bind<CalculateRequirementsUseCase>(TYPES.CALCULATE_REQUIREMENTS_USE_CASE).to(CalculateRequirementsUseCase).inTransientScope();
  container.bind<SyncSportsMenuUseCase>(TYPES.SYNC_SPORTS_MENU_USE_CASE).to(SyncSportsMenuUseCase).inTransientScope();

  // Repositories
  container.bind<IProductionRepository>(TYPES.PRODUCTION_REPOSITORY).to(FirebaseProductionRepository).inSingletonScope();
  container.bind<IPurchasingRepository>(TYPES.PURCHASING_REPOSITORY).to(FirebasePurchasingRepository).inSingletonScope();
  container.bind<GetScheduleUseCase>(TYPES.GetScheduleUseCase).to(GetScheduleUseCase).inTransientScope();
  container.bind<GetEventsUseCase>(TYPES.GetEventsUseCase).to(GetEventsUseCase).inTransientScope();
  container.bind<ImportEventsUseCase>(TYPES.ImportEventsUseCase).to(ImportEventsUseCase).inTransientScope();

  // Employee Module
  container.bind<IEmployeeRepository>(TYPES.EmployeeRepository).to(FirebaseEmployeeRepository).inSingletonScope();
  container.bind<GetEmployeesUseCase>(TYPES.GetEmployeesUseCase).to(GetEmployeesUseCase).inTransientScope();
  container.bind<SaveEmployeeUseCase>(TYPES.SaveEmployeeUseCase).to(SaveEmployeeUseCase).inTransientScope();
  container.bind<DeleteEmployeeUseCase>(TYPES.DeleteEmployeeUseCase).to(DeleteEmployeeUseCase).inTransientScope();

  // Schedule & Events Repositories
  container.bind<IShiftRepository>(TYPES.ShiftRepository).to(FirebaseShiftRepository).inSingletonScope();
  container.bind<IEventRepository>(TYPES.EventRepository).to(FirebaseEventRepository).inSingletonScope();
}
