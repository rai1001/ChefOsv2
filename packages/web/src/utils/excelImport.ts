 ...
 */
export const processFileForAnalysis = async (
  file: File,
    ...
    reader.onload = (e) => {
      try {
        ...
        for (const sheetName of workbook.SheetNames) {
          ...
          const isMasterList =
            sheetName.toUpperCase().includes('LIST. PRODUCTOS') ||
          ...
          if (isMasterList) {
            ...
            for (let r = range.s.r; r <= Math.min(range.e.r, 50); r++) {
              ...
              for (let c = range.s.c; c <= range.e.c; c++) {
                ...
                if (cell?.v) rowText.push(String(cell.v).toUpperCase().trim());
              }

              if (rowText.length === 0) continue;
              const rowString = rowText.join(' ');

              const strongRecipeTitles = [
              ...
              const strongIngredientTitles = [
                'LISTA DE PRODUCTOS',
                'MASTER PRODUCTOS',
                'LISTADO DE PRECIOS',
              ];
              const hasStrongIngredients = strongIngredientTitles.some((k) =>
                rowString.includes(k)
              );
              ...
              const isRecipeHeader =
                rowText.some((t) =>
                  ['INGREDIENTE', 'PRODUCTO', 'DESCRIPCIÓN'].some((k) => t.includes(k))
                ) &&
                rowText.some((t) => ['CANTIDAD', 'PESO', 'NETO', 'Q'].some((k) => t.includes(k)));

              const isIngredientHeader =
                rowText.some((t) =>
                  ['PRODUCTO', 'NOMBRE', 'DESCRIPCIÓN'].some((k) => t.includes(k))
                ) &&
                rowText.some((t) => ['PRECIO', 'COSTE', 'COST', '€'].some((k) => t.includes(k)));

              const isInventoryHeader =
                rowText.some((t) =>
                  ['PRODUCTO', 'NOMBRE', 'DESCRIPCIÓN'].some((k) => t.includes(k))
                ) &&
                rowText.some((t) =>
                  ['CANTIDAD', 'STOCK', 'QTY', 'CONTEO', 'UNIDADES'].some((k) => t.includes(k))
                );

              const isMenuHeader = rowText.some((t) =>
                ['MENÚ', 'MENU', 'PLATO'].some((k) => t.includes(k))
              );
          ...
          if (type === 'INGREDIENTS') {
            if (isMasterList) {
              ...
              let unitCol = 'C';
              let priceCol = 'D';
              const allergenCols: Record<string, string> = {};
              ...
              if (bestHeaderRowIdx !== -1) {
                ...
                Object.entries(headerRow).forEach(([col, val]) => {
                  ...
                  const matchedAllergen = ALLERGENS.find(
                    (a) => normalize(a) === v || (normalize(a).includes(v) && v.length > 3)
                  );
              ...
              for (let i = dataStartIdx; i < rows.length; i++) {
                ...
                if (['PRODUCTO', 'DESCRIPCION', 'NOMBRE', 'UNIDAD', 'PRECIO'].includes(nameUpper))
                  continue;

                const unit = String(rawUnit || 'kg')
                  .toLowerCase()
                  .trim();
                const finalUnit =
                  unit === 'ud' || unit === 'u.' || unit === 'u' || unit === 'un' || unit === 'uni'
                    ? 'un'
                    : unit;
                ...
                Object.entries(allergenCols).forEach(([col, label]) => {
                  const val = String(row[col] ?? '')
                    ...
                    .trim();
                  if (
                    ['SI', 'SÍ', 'X', 'TRAZAS', 'S', '1', 'YES'].includes(val) ||
                    val.includes('SI') ||
                    val.includes('SÍ')
                  ) {
              ...
              json.forEach((row) => {
                ...
                const findKey = (candidates: string[]) =>
                  keys.find((k) => candidates.some((c) => k.toUpperCase().includes(c)));
                ...
                const price = priceKey
                  ? Number(
                      String(row[priceKey] ?? '0')
                        .replace(/[^\d,.]/g, '')
                        .replace(',', '.')
                    )
                  : 0;
                let unit = unitKey
                  ? String(row[unitKey] ?? 'kg')
                      .toLowerCase()
                      .trim()
                  : 'kg';
            ...
            const isBaseSheet =
              sheetName.toUpperCase().includes('BASES') || sheetName.toUpperCase().includes('BASE');
            let recipeName = '';
            const nameRowIdx = Math.max(0, headerRow - 2);
            for (let c = range.s.c; c <= range.e.c; c++) {
              const cell = sheet[XLSX.utils.encode_cell({ r: nameRowIdx, c })];
              if (cell?.v) recipeName += cell.v + ' ';
            }
            ...
            if (!recipesMap.has(recipeKey)) {
              ...
              json.forEach((row) => {
                const rawName =
                  row['DESCRIPCIÓN PRODUCTO PROVEEDOR'] || row['DESCRIPCIÓN'] || row.PRODUCTO;
            ...
            json.forEach((row) => {
              ...
              const foundRecipe = Array.from(recipesMap.values()).find(
                (r) => normalize(r.name) === normalize(String(dishName))
              );
              if (foundRecipe) {
