...
const dispatchMockUpdate = (collectionName: string) => {
    if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('MOCK_DB_UPDATED', { detail: { collection: collectionName } }));
    }
};
...
export const updateDocument = async <T extends DocumentData>(collectionName: string, id: string, data: UpdateData<T>): Promise<void> => {
    const mockDB = getMockDB();
    if (mockDB) {
...
export const queryDocuments = async <T extends DocumentData>(collectionRef: CollectionReference<T>, ...constraints: QueryConstraint[]): Promise<T[]> => {
    const mockDB = getMockDB();
    if (mockDB) {
        const collectionName = collectionRef.path;
