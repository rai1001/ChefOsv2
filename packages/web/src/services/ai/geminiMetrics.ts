...
const COST_PER_1K_TOKENS = COST_PER_1M_INPUT_TOKENS / 1000;
...
export async function trackedGeminiCall<T>(
    ...
    try {
        ...
        const budget = await checkBudgetBeforeCall(metadata.outletId, feature, estimatedInputTokens * COST_PER_1K_TOKENS);
        console.log(`[AI Metrics] Budget check result:`, budget);
    ...
    try {
        ...
        if (error instanceof TypeError && error.message.includes('success')) {
            console.error(`[AI Metrics] DETECTED TYPEERROR (reading success) for ${feature}. Check callback return type.`);
        }
        success = false;
        errorMsg = error.message;
        throw error;
    } finally {
        console.log(`[AI Metrics] Entering finally block for ${feature}. Success: ${success}, CacheHit: ${cacheHit}`);
        if (!cacheHit) {
            try {
                ...
                logMetricToRealtime(metric).catch(err => console.error("[AI Metrics] RTDB Log Error:", err));
                logMetricToFirestore(metric).catch(err => console.error("[AI Metrics] Firestore Log Error:", err));

                if (success) {
                    updateUsageAfterCall(metadata.outletId, actualCost, feature).catch(err => console.error("[AI Metrics] Budget Update Error:", err));
                    if (result && cacheKey) {
                        setCachedResult(cacheKey, feature, inputPayload, result).catch(err => console.error("[AI Metrics] Cache Set Error:", err));
                    }
                }
            } catch (finalError) {
