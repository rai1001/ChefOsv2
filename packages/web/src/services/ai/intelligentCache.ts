    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (now < expiresAt) {
                ...
                setDoc(docRef, { hitCount: (data.hitCount || 0) + 1 }, { merge: true }).catch(console.error);
 ...
 */
export async function setCachedResult(key: string, feature: AIFeature, input: any, result: any): Promise<void> {
    try {
