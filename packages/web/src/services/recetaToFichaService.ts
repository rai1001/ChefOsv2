...
export async function convertirRecetaAFicha(
  ...
  const ingredients = await firestoreService.getAll<Ingredient>(
    collections.ingredients as unknown as CollectionReference<Ingredient>
  );
  ...
  return {
    ...
    pasos:
      receta.instructions?.map((inst, idx) => ({
        id: `step-${idx}`,
        orden: idx + 1,
        descripcion: inst,
        tiempoEstimado: 0,
...
export async function importarRecetasMasivo(
  ...
  for (let i = 0; i < recetaIds.length; i++) {
    ...
    try {
      ...
      resultados.fallidas.push({ recetaId, error: message });
    }
