...
export const createIngredientSlice: StateCreator<AppState, [], [], IngredientSlice> = (
  set,
  get
) => ({
  ingredients: [],
  ...
  updateIngredient: async (updatedIngredient: Ingredient) => {
    ...
    if (current && current.costPerUnit !== updatedIngredient.costPerUnit) {
      ...
      newHistory.push({
        ...
        price: updatedIngredient.costPerUnit || 0,
    ...
    set((state) => ({
      ingredients: state.ingredients.map((i) =>
        i.id === updatedIngredient.id ? finalIngredient : i
      ),
    }));
