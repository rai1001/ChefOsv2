...
const Sparkline = ({ data, color }: { data: number[]; color: string }) => (
  <div className="flex items-end gap-1 h-8">
    {data.map((h, i) => (
      <div key={i} className={`w-1 rounded-t-sm ${color} opacity-60`} style={{ height: `${h}%` }} />
    ))}
  </div>
);

export const SupplierPage: React.FC = () => {
  const { suppliers, addSupplier, updateSupplier, deleteSupplier, ingredients, activeOutletId } =
    useStore();
  ...
  const handleClearData = async () => {
    if (!window.confirm('¿ESTÁS SEGURO? Esto borrará TODOS los proveedores permanentemente.'))
      return;
  ...
  const [viewingIngredientsSupplier, setViewingIngredientsSupplier] = useState<Supplier | null>(
    null
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 px-5 py-3 rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95"
          >
            {isUploadingInvoice ? (
              <>
                              ...
                              <table className="w-full text-left">
                                <thead>
                                  <tr className="border-b border-white/5">
                                    <th className="px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                      Fecha
                                    </th>
                                    <th className="px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                      Precio
