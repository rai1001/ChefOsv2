...
import { AIInventoryAdvisor } from '@/presentation/components/inventory/AIInventoryAdvisor';
import { LabelPrinterModal } from '@/presentation/components/printing/LabelPrinterModal';
...
export const InventoryPage: React.FC = () => {
  ...
  const handleClearData = async () => {
    ...
    if (
      !window.confirm(
        '¿ESTÁS SEGURO? Esto borrará el stock de todos los productos en ESTA cocina. Los ingredientes no se eliminarán, solo su existencia en inventario.'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const scopedInventory = inventory.filter(
        (item) => !activeOutletId || item.outletId === activeOutletId
      );
  ...
  const handleBarcodeScan = async (barcode: string) => {
    ...
    try {
      const existingIngredient = ingredients.find((ing) => ing.defaultBarcode === barcode);
      ...
      if (existingIngredient) {
        setProductLookup({
          ...
          nutritionalInfo: existingIngredient.nutritionalInfo
            ? {
                calories: existingIngredient.nutritionalInfo.values.energyKcal,
                protein: existingIngredient.nutritionalInfo.values.protein,
                carbs: existingIngredient.nutritionalInfo.values.carbohydrates,
                fat: existingIngredient.nutritionalInfo.values.fat,
              }
            : undefined,
        });
        setBatchForm({
          quantity: '',
          expiryDate: '',
          costPerUnit: (existingIngredient.costPerUnit || 0).toString(),
        });
      } else {
  ...
  const handleConfirmBatch = async () => {
    ...
    try {
      ...
      const existingInventoryItem = inventory.find(
        (inv) =>
          (inv.name === productLookup.name || inv.id === scannedBarcode) &&
          inv.outletId === activeOutletId
      );
  ...
  const filteredIngredients = React.useMemo(() => {
    ...
    if (activeFilter !== 'all') {
      if (activeFilter === 'low-stock') {
        items = items.filter((item: InventoryItem) => (item.stock || 0) <= (item.minStock || 0));
      } else if (activeFilter === 'expiring') {
        items = items.filter((item: InventoryItem) => {
          const batches = item.batches || [];
          return batches.some((batch: IngredientBatch) => {
            const daysUntilExpiry = Math.floor(
              (new Date(batch.expiresAt).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
            );
            return daysUntilExpiry <= 7 && daysUntilExpiry >= 0;
          });
        });
      } else {
        items = items.filter((item: InventoryItem) => item.category === activeFilter);
      }
    }
    return items;
  ...
  const expiringCount = React.useMemo(() => {
    const outletInventory = inventory.filter(
      (item: InventoryItem) => !activeOutletId || item.outletId === activeOutletId
    );
    return outletInventory.filter((item: InventoryItem) => {
      const batches = item.batches || [];
      return batches.some((batch: IngredientBatch) => {
  ...
  const handleImportComplete = async (data: any) => {
    ...
    if (data.items && Array.isArray(data.items)) {
      ...
      for (const item of data.items) {
        ...
        if (match) {
          try {
            ...
            const existingInventoryItem = inventory.find(
              (inv: InventoryItem) =>
                inv.ingredientId === match.id && inv.outletId === activeOutletId
            );

            if (existingInventoryItem) {
              const updatedBatches = [...existingInventoryItem.batches, newBatch];
              const newStock = updatedBatches.reduce(
                (sum: number, b: IngredientBatch) => sum + b.currentQuantity,
                0
              );
              // Remove unused NextExpiry calculation or use it
              // const _nextExpiry = existingInventoryItem.batches && existingInventoryItem.batches.length > 0
              //     ? existingInventoryItem.batches[0]
              //     : null;

              // The original `nextExpiry` calculation was unused.
              // If the intent was to update the `nextExpiry` field in `InventoryItem`,
              // it should be included in the `updateInventoryItem` call.
              // For now, we'll just remove the unused calculation.
              // If a `nextExpiry` field is needed, it should be added to InventoryItem type
              // and calculated here.
      ...
      if (updatedCount > 0) {
        alert(
          `Inventario actualizado: ${updatedCount} productos procesados.` +
            (missedCount > 0 ? ` (${missedCount} no encontrados)` : '')
        );
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 min-h-screen bg-transparent text-slate-100 fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-slate-500 text-xs font-bold mt-2 tracking-[0.3em] uppercase">
            Gestión de Existencias Inteligente v2.0
          </p>
        </div>
          ...
          <div>
            ...
            <p className="text-2xl font-black text-white font-mono">
              {inventory
                ...
                .reduce((acc: number, item: InventoryItem) => {
      ...
      <div className="premium-glass p-2 flex flex-col xl:flex-row gap-4 justify-between items-center rounded-2xl">
        <div className="flex gap-1 overflow-x-auto max-w-full pb-1 custom-scrollbar">
          ...
          >
            ...
            Bajo Stock
            {lowStockCount > 0 && (
              <span className="bg-white/20 px-1.5 py-0.5 rounded text-[9px] ml-1">
                {lowStockCount}
              </span>
            )}
          </button>
          ...
          <div className="flex gap-1">
            {[
              ...
              <button
                ...
                className={`px-4 py-3 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all border whitespace-nowrap ${
                  activeFilter === cat.id
        ...
        <div className="w-full xl:w-96 relative group">
          <Search
            className="absolute left-4 top-1/2 transform -translate-y-1/2 text-slate-500 group-focus-within:text-primary transition-colors"
            size={16}
          />
          <input
            type="text"
        ...
        <div className="overflow-x-auto flex-1 custom-scrollbar">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-white/5">
              {filteredIngredients.map((ing: InventoryItem, _idx: number) => {
                const totalStock = ing.stock || 0;
                const totalValue =
                  ing.batches?.reduce(
                    (acc: number, b: IngredientBatch) => acc + b.currentQuantity * b.costPerUnit,
                    0
                  ) || totalStock * ing.costPerUnit;
                const minStock = ing.minStock || 0;
                const isLowStock = totalStock <= minStock;
                const nearExpiryBatches =
                  ing.batches?.filter(
                    (b: IngredientBatch) => getDaysUntilExpiry(b.expiresAt) <= 7
                  ) || [];
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="inline-flex flex-col items-center">
                          <span
                            className={`font-mono font-black text-lg ${isLowStock ? 'text-rose-400' : 'text-slate-200'}`}
                          >
                          ...
                          <div className="bg-background/50 rounded-2xl border border-white/5 overflow-hidden shadow-2xl">
                            ...
                            <table className="w-full text-sm">
                              ...
                              <tbody className="divide-y divide-white/[0.02]">
                                {ing.batches
                                  ?.sort(
                                    (a: IngredientBatch, b: IngredientBatch) =>
                                      new Date(a.expiresAt).getTime() -
                                      new Date(b.expiresAt).getTime()
                                  )
                                  .map((batch: IngredientBatch) => {
                                    ...
                                    return (
                                      <tr
                                        key={batch.id}
                                        className="hover:bg-white/[0.03] transition-colors group/batch"
                                      >
                                        ...
                                        <td className="px-6 py-4">
                                          <div
                                            className={`font-bold ${isExpired ? 'text-rose-400' : isNearExpiry ? 'text-amber-400' : 'text-slate-200'}`}
                                          >
                                          ...
                                          <div className="text-[10px] text-slate-500 uppercase font-black">
                                            {daysToExpiry === 0
                                              ...
                                              : daysToExpiry < 0
                                                ...
                                                : `Quedan ${daysToExpiry} días`}
                                          </div>
                                        </td>
                                        <td className="px-6 py-4">
                                          <span className="font-bold text-slate-200 font-mono text-base">
                                            {batch.currentQuantity}
                                          </span>
                                          <span className="text-[10px] text-slate-500 font-bold uppercase ml-1.5">
                                            {ing.unit}
                                          </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                          {isExpired ? (
                                            <span className="inline-flex px-2 py-0.5 rounded-full text-[9px] font-black bg-rose-500 text-white uppercase">
                                              Vencido
                                            </span>
                                          ) : isNearExpiry ? (
                                            <span className="inline-flex px-2 py-0.5 rounded-full text-[9px] font-black bg-amber-500 text-black uppercase animate-pulse">
                                              Alerta
                                            </span>
                                          ) : (
                                            <span className="inline-flex px-2 py-0.5 rounded-full text-[9px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 uppercase">
                                              Activo
                                            </span>
                                          )}
                                        </td>
                                ...
                                {(!ing.batches || ing.batches.length === 0) && (
                                  <tr>
                                    <td
                                      colSpan={6}
                                      className="px-6 py-10 text-center text-slate-600 text-xs italic"
                                    >
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-24 text-center">
                    ...
                    <div className="text-slate-600 text-sm mt-1 max-w-xs mx-auto">
                      {searchTerm
                        ? `No encontramos nada parecido a "${searchTerm}"`
                        : 'Pronto aparecerán tus productos aquí.'}
                    </div>
                  </td>
                </tr>
              )}
      ...
      {scanStep === 'scanning-barcode' && (
        <BarcodeScanner onScan={handleBarcodeScan} onClose={() => setScanStep('idle')} />
      )}
            ...
            {isLookingUp ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-primary/5 p-4 rounded-xl border border-primary/20">
                  ...
                  {productLookup.brand && (
                    <>
                      <p className="text-sm text-slate-500 mt-3 uppercase tracking-wider font-semibold">
                        Marca
                      </p>
              ...
              <div className="space-y-4">
                <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-4">
                  <p className="text-slate-300">
                    ...
                    <span className="font-mono font-bold text-white">{scannedBarcode}</span>
                  </p>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-surface rounded-2xl p-6 w-full max-w-md border border-white/10 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-6 bg-primary/5 p-4 rounded-xl border border-primary/20">
              ...
              <div className="text-sm text-slate-500 mt-4 uppercase tracking-wider font-semibold">
                Fecha de Caducidad
              </div>
