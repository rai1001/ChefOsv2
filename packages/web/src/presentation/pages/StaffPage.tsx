...
import { SaveEmployeeUseCase } from '@/application/use-cases/employee/SaveEmployeeUseCase';
...
export const StaffPage: React.FC = () => {
  ...
  const saveEmployeeUseCase = useInjection<SaveEmployeeUseCase>(TYPES.SaveEmployeeUseCase);
  const deleteEmployeeUseCase = useInjection<DeleteEmployeeUseCase>(TYPES.DeleteEmployeeUseCase);
  ...
  const stats = useMemo(() => {
    ...
    const vacation = staff.filter((e) => {
      ...
      return e.vacationDates && Array.isArray(e.vacationDates)
        ? e.vacationDates.includes(today)
        : false;
    }).length;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Construct object compatible with Domain Employee
      const employeeData = {
        ...formData,
        outletId: activeOutletId,
        id: editingEmployee?.id || crypto.randomUUID(),
        active: formData.status === 'ACTIVE', // Required by Domain Entity
        status: formData.status || 'ACTIVE',
      };

      // We pass it to the use case. Types might conflict if we don't cast explicitly
      // but since use case expects Domain Employee, we ensure it matches.
      // @ts-ignore - Resolving type mismatch between generic and domain Employee
      await saveEmployeeUseCase.execute(employeeData);
      ...
      <div className="premium-glass p-8 rounded-3xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {staff.map((emp) => (
            <div
              key={emp.id}
              className="bg-black/20 rounded-2xl p-5 border border-white/5 hover:bg-white/[0.02] transition-colors"
            >
              ...
              {emp.qualificationDocs && emp.qualificationDocs.length > 0 ? (
                <div className="space-y-2">
                  {emp.qualificationDocs.map((doc, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between text-xs bg-white/5 p-3 rounded-xl border border-white/5 hover:border-primary/30 transition-colors group"
                    >
                      <a
                        href={doc.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-slate-300 hover:text-primary transition-colors truncate flex-1 font-medium"
                      >
                        {doc.name}
                      </a>
                      {doc.expiryDate && (
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ml-2 ${new Date(doc.expiryDate) < new Date() ? 'bg-red-500/20 text-red-400' : 'bg-slate-500/20 text-slate-400'}`}
                        >
                          Exp: {doc.expiryDate}
                        </span>
                      )}
                ...
                <div className="text-[10px] text-slate-600 font-bold uppercase tracking-widest italic py-2">
                  Sin documentos
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, vacationDaysTotal: parseInt(e.target.value) || 0 })
                    }
                  />
                </div>
                <div>
                  ...
                  {(formData.qualificationDocs || []).map((doc, idx) => (
                    <div key={idx} className="flex gap-2 items-center">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          const newDocs = (formData.qualificationDocs || []).filter(
                            (_, i) => i !== idx
                          );
                          setFormData({ ...formData, qualificationDocs: newDocs });
                        }}
