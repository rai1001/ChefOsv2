          ...
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-slate-500 text-xs font-bold mt-1 tracking-widest uppercase">
                Aprovisionamiento Directo de Inventario
              </p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-end">
              ...
              <div className="bg-primary/10 border border-primary/20 rounded-2xl p-4 flex items-center gap-4">
                ...
                <p className="text-[10px] text-slate-300 font-medium leading-relaxed">
                  Las compras manuales se guardarán como borradores listos para aprobación o
                  recepción inmediata.
                </p>
              </div>
            </div>
              ...
              <div className="max-h-[350px] overflow-y-auto space-y-4 pr-3 custom-scrollbar min-h-[100px]">
                {items.map((item, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="group relative bg-white/[0.02] hover:bg-white/[0.05] p-5 rounded-2xl border border-white/5 transition-all duration-300"
                    >
                      <div className="grid grid-cols-12 gap-5 items-end">
                        <div className="col-span-12 lg:col-span-5 space-y-2">
                          <label className="block text-[9px] font-black uppercase text-slate-500 tracking-widest">
                          ...
                          <Select
                            ...
                            value={selectedIng}
                            onChange={(option: any) =>
                              updateItem(index, { ingredientId: option?.value || '' })
                            }
                            formatOptionLabel={(opt: any) => (
                              <div className="flex justify-between items-center py-0.5">
                                ...
                                <div className="flex items-center gap-3">
                                  <span
                                    className={`text-[9px] px-1.5 py-0.5 rounded uppercase font-black ${opt.stock <= opt.minStock ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}
                                  >
                        ...
                        <div className="col-span-5 lg:col-span-3 space-y-2">
                          <label className="block text-[9px] font-black uppercase text-slate-500 tracking-widest flex justify-between">
                            ...
                            {selectedIng && (
                              <span
                                className={`flex items-center gap-1 ${isPriceHigher ? 'text-red-400' : 'text-emerald-400'}`}
                              >
                                {isPriceHigher ? (
                                  <TrendingUp size={10} />
                                ) : (
                                ...
                                {Math.abs(
                                  (((item.costPerUnit || 0) - selectedIng.cost) /
                                    selectedIng.cost) *
                                    100
                                ).toFixed(0)}
                                %
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) =>
                                updateItem(index, { costPerUnit: Number(e.target.value) })
                              }
                            />
            ...
            <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-white/10 gap-6">
              <div className="flex items-center gap-4 bg-white/5 px-6 py-4 rounded-3xl border border-white/5">
                ...
                <div className="text-3xl font-black text-white font-mono flex items-center gap-1">
                  {items
                    .reduce((sum, item) => sum + (item.quantity || 0) * (item.costPerUnit || 0), 0)
                    .toFixed(2)}
