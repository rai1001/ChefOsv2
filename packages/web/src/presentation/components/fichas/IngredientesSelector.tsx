...
export const IngredientesSelector: React.FC<Props> = ({
  ...
  const searchResults = useMemo(() => {
    ...
    return (fuse.search(searchTerm) as any[]).slice(0, 5).map((res) => res.item as Ingredient);
  }, [searchTerm, fuse]);
  ...
  const handleSelect = (ing: Ingredient) => {
    onAdd({
      ...
      cantidad: 0,
      unidad: ing.unit,
      costoUnitario: ing.costPerUnit || 0,
      costoTotal: 0,
      esOpcional: false,
    });
    setSearchTerm('');
            ...
            {searchResults.map((ing) => (
              ...
              >
                ...
                <span className="text-xs text-slate-500">
                  {(ing.costPerUnit || 0).toFixed(2)}€/{ing.unit}
                </span>
              </button>
            ))}
          </div>
      ...
      <div className="space-y-2">
        {ingredientes.map((ing, idx) => (
          <div
            ...
            className="flex items-center gap-3 bg-surface p-3 rounded-lg border border-white/5"
          >
            ...
            <div className="w-24 text-right">
              <span className="text-xs font-mono text-primary">{ing.costoTotal?.toFixed(2)} €</span>
            </div>
