...
export const SportsMenuReviewModal: React.FC<SportsMenuReviewModalProps> = ({
  event,
  data,
  onClose,
  onSyncComplete,
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white uppercase tracking-tight">
                Revisión de Menú Digitalizado
              </h2>
              <p className="text-xs text-slate-400 font-medium">
                Vincula los platos detectados con tu base de datos
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="grid grid-cols-1 gap-4">
            {items.map((item) => (
              <div
                key={item.uid}
                className="premium-glass p-4 grid grid-cols-12 gap-4 items-center group"
              >
                {/* AI Data */}
                <div className="col-span-3">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    {item.quantity || event.pax} {item.unit || 'Pax'}
                  </p>
                </div>
                ...
                <div className="col-span-5">
                  ...
                  <SearchableMatchSelector
                    ...
                    onSelect={(option) =>
                      setMatches((prev) => ({
                        ...prev,
                        [item.uid]: { id: option.id, type: option.type },
                      }))
                    }
                    placeholder="Buscar..."
                  />
                ...
                <div className="col-span-4 flex justify-end gap-2 text-center">
                  <button
                    ...
                    className={`flex-1 p-3 rounded-xl border flex flex-col items-center justify-center transition-all ${
                      selectedTasks.includes(item.uid)
                  ...
                  <button
                    ...
                    className={`flex-1 p-3 rounded-xl border flex flex-col items-center justify-center transition-all ${
                      selectedPurchases.includes(item.uid)
