...
export const SearchableMatchSelector: React.FC<SearchableMatchSelectorProps> = ({
    ...
    const options = useMemo(() => {
        const r: MatchOption[] = recipes.map(x => ({ id: x.id, name: x.name, type: 'recipe', category: x.category }));
        const i: MatchOption[] = ingredients.map(x => ({ id: x.id, name: x.name, type: 'ingredient', category: x.category }));
        return [...r, ...i];
    }, [recipes, ingredients]);
        ...
        <div className="relative w-full">
            ...
            >
                <div className="flex items-center gap-2 truncate">
                    {selectedOption ? (
                        <>
                            {selectedOption.type === 'recipe' ? <Utensils size={14} className="text-purple-400" /> : <Zap size={14} className="text-emerald-400" />}
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 z-[120] premium-glass border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="max-h-60 overflow-y-auto">
                            {filteredOptions.map(option => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`p-1.5 rounded-lg ${option.type === 'recipe' ? 'bg-purple-500/10 text-purple-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
                                            {option.type === 'recipe' ? <Utensils size={14} /> : <Zap size={14} />}
                                        </div>
                                        <div>
                                            <p className="text-sm text-white font-medium leading-none">{option.name}</p>
                                            <p className="text-[10px] text-slate-500 uppercase mt-1 tracking-wider">{option.type} • {option.category || 'Sin categoría'}</p>
                                        </div>
                                    </div>
                            ...
                            {filteredOptions.length === 0 && (
                                <div className="p-4 text-center text-slate-500 text-xs italic">No se encontraron resultados</div>
                            )}
                        </div>
                    </div>
                </>
