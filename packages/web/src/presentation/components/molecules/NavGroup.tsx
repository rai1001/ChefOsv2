    ...
    <div className="space-y-1">
      <button
        ...
        className={`w-full flex items-center justify-between px-3 py-2 rounded-xl hover:bg-white/5 transition-colors group ${
          isAnyChildActive ? 'text-white' : 'text-slate-500'
        }`}
      >
        <div className="flex items-center gap-3">
          {React.cloneElement(icon as React.ReactElement<any>, {
            size: 18,
            className: isAnyChildActive ? 'text-primary' : '',
          })}
          <span
            className={`font-semibold text-[10px] uppercase tracking-[0.1em] ${isAnyChildActive ? 'text-slate-200' : 'text-slate-500'}`}
          >
            {label}
          </span>
        </div>
