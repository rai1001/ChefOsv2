  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      if (type === 'AUDIT') {
        ...
        const registerUseCase = container.get<RegisterStockMovementUseCase>(
          TYPES.RegisterStockMovementUseCase
        );
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-lg bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 grid grid-cols-3 gap-2 border-b border-white/5">
          {(['PURCHASE', 'USAGE', 'WASTE', 'AUDIT', 'ADJUSTMENT'] as StockTransactionType[]).map(
            (t) => (
              <button
                key={t}
                onClick={() => setType(t)}
                className={`flex flex-col items-center gap-1 p-3 rounded-xl border transition-all text-[10px] font-bold uppercase tracking-wider ${
                  type === t
          ...
          {type === 'PURCHASE' && (
            <div>
              ...
              <p className="text-[10px] text-slate-500 mt-1">
                Coste unitario actual: {ingredient.costPerUnit}€
              </p>
            </div>
          )}
        ...
        <div className="p-6 border-t border-white/10 bg-white/5 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-xl font-bold text-xs uppercase tracking-widest text-slate-400 hover:bg-white/5 transition-colors"
          >
            Cancelar
          </button>
          <button
